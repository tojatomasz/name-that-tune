...
const normalize = (str: string | undefined, keepSpaces: boolean = false) => {
  ...
  // Remove everything (but not spaces) that is not a number, letter, or from Cyrylic/Polish/Arabic/Hebrew alphabet
  // (Github Copilot says Arabic letters range from \u0621 to \u064A and Hebrew letters range from \u05D0 to \u05EA)
  cleaned = cleaned.replace(/[^\wа-яА-ЯіїІЇ\dąćęłńóśźż\u0621-\u064A\u05D0-\u05EA\s\d]/g, '');
